            ...
            <div className="max-w-[960px] w-full mx-auto flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-sm text-muted-foreground">{t("currentLabel")}</p>
                </div>
              </div>
                ...
                <form className="flex flex-col gap-4">
                  <div className="flex gap-4 flex-col md:flex-row">
                    <div className="flex-1 flex flex-col gap-2">
                      <Label>{t("teaching.certificateLabel")}</Label>
                      <Controller
                        name="certificateType"
                        control={control}
                        render={({ field }) => (
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                      <Label>{t("teaching.yearLabel")}</Label>
                      ...
                      {errors.teachingYear && <p className="text-sm text-destructive">{errors.teachingYear.message}</p>}
                    </div>
                  </div>

                  <div className="flex flex-col gap-2">
                    <Label className="text-sm font-semibold">{t("teaching.uploadTitle")}</Label>
                    <div className="border-2 border-dashed rounded-lg p-6 flex flex-col items-center gap-3 hover:border-primary/50 transition-colors cursor-pointer" onClick={() => document.getElementById("teachingFile")?.click()}>
                      <Upload className="w-8 h-8 text-muted-foreground" />
                      <div className="text-center">
                        <p className="text-sm font-medium">{t("teaching.uploadPrompt")}</p>
                        <p className="text-xs text-muted-foreground">{t("teaching.uploadHint")}</p>
                      </div>
                      {certificationMerged.teachingCertificate.file.fileName && (
                        <p className="text-sm text-primary font-medium">{certificationMerged.teachingCertificate.file.fileName}</p>
                      )}
                    </div>
                    <input type="file" id="teachingFile" accept={ACCEPT_CERT} onChange={handleTeachingFileChange} className="hidden" />
                    {errors.teachingCertificateFile && <p className="text-sm text-destructive">{errors.teachingCertificateFile.message}</p>}

                    <div className="mt-2 rounded-lg p-3 bg-muted/50 flex gap-2 items-start">
                      <Info className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
                      <div className="flex-1 flex flex-col gap-1">
                        <p className="text-sm font-semibold text-primary">{t("teaching.reviewTitle")}</p>
                        <p className="text-sm text-muted-foreground">{t("teaching.reviewDescription")}</p>
                      </div>
                    </div>
                  </div>
                </form>
              ...
              <div ref={educationCardRef} className="bg-card rounded-xl p-6 flex flex-col gap-5 border shadow-sm">
                ...
                <form className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <Label className="text-sm font-semibold">{t("education.uploadTitle")}</Label>
                    <div className="border-2 border-dashed rounded-lg p-6 flex flex-col items-center gap-3 hover:border-primary/50 transition-colors cursor-pointer" onClick={() => document.getElementById("educationFile")?.click()}>
                      <Upload className="w-8 h-8 text-muted-foreground" />
                      <div className="text-center">
                        <p className="text-sm font-medium">{t("education.uploadPrompt")}</p>
                        <p className="text-xs text-muted-foreground">{t("education.uploadHint")}</p>
                      </div>
                      {certificationMerged.higherEducation.file.fileName && (
                        <p className="text-sm text-primary font-medium">{certificationMerged.higherEducation.file.fileName}</p>
                      )}
                    </div>
                    <input type="file" id="educationFile" accept={ACCEPT_CERT} onChange={handleEducationFileChange} className="hidden" />
                    {errors.educationFile && <p className="text-sm text-destructive">{errors.educationFile.message}</p>}
                  </div>
                </form>
              </div>
            </div>
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-t shadow-lg py-4 z-10">
          <div className="max-w-[960px] w-full mx-auto px-4 md:px-6">
            <div className="flex justify-between items-center">
              <Button variant="outline" onClick={() => router.push("/become-tutor/photo")} className="gap-2">
                <ArrowLeft className="w-4 h-4" />
                {t("back")}
              </Button>
              <Button type="submit" size="lg" disabled={isUploading} onClick={handleSubmit(onSubmit, onValidationError)} className="gap-2">
                {t("continue")}
