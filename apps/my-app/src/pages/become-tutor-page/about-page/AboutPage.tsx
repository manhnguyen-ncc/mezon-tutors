        ...
        <div className="flex-1 overflow-y-auto pb-28">
          <div className="py-6 px-4 md:py-5 md:px-6">
            <div className="max-w-[960px] w-full mx-auto flex flex-col gap-6">
              ...
              <div ref={formCardRef} className="bg-card rounded-xl p-6 flex flex-col gap-6 border shadow-sm">
                <div className="flex flex-col gap-2">
                  <h2 className="text-2xl font-bold">{t("title")}</h2>
                  <p className="text-muted-foreground text-sm">{t("subtitle")}</p>
                </div>

                <form onSubmit={handleSubmit(onSubmit, onValidationError)} className="flex flex-col gap-4">
                  <div className="flex gap-4 flex-col md:flex-row">
                    <div className="flex-1 flex flex-col gap-2">
                      <Label htmlFor="firstName">{t("fields.firstNameLabel")}</Label>
                      <Input id="firstName" placeholder={t("fields.firstNamePlaceholder")} {...register("firstName")} />
                      {errors.firstName && <p className="text-sm text-destructive">{errors.firstName.message}</p>}
                    </div>
                    <div className="flex-1 flex flex-col gap-2">
                      <Label htmlFor="lastName">{t("fields.lastNameLabel")}</Label>
                      <Input id="lastName" placeholder={t("fields.lastNamePlaceholder")} {...register("lastName")} />
                  ...
                  <div className="flex gap-4 flex-row">
                    <div className="flex-1 flex flex-col gap-2">
                      ...
                      <Controller name="country" control={control} render={({ field }) => (
                        <Select value={field.value} onValueChange={field.onChange}>
                          <SelectTrigger><SelectValue placeholder={t("fields.countryPlaceholder")} /></SelectTrigger>
                          <SelectContent>
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                      <Label htmlFor="phone">{t("fields.phoneLabel")}</Label>
                  ...
                  <div className="flex flex-col gap-2">
                    <Label>{t("fields.subjectLabel")}</Label>
                    <Controller name="subject" control={control} render={({ field }) => (
                      <Select value={field.value} onValueChange={field.onChange}>
                        <SelectTrigger><SelectValue placeholder={t("fields.subjectPlaceholder")} /></SelectTrigger>
                        <SelectContent>
                    ...
                    <p className="text-sm text-muted-foreground">{t("fields.subjectHelper")}</p>
                    {errors.subject && <p className="text-sm text-destructive">{errors.subject.message}</p>}
                  </div>

                  <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <p className="text-sm text-muted-foreground">{t("fields.languagesLabel")}</p>
                      <button type="button" onClick={() => append({ language: "", proficiency: "" })} className="text-sm text-primary hover:opacity-80 cursor-pointer">+ {t("addAnotherLanguage")}</button>
                    </div>
                    {fields.map((field, index) => (
                      <div key={field.id} className="flex gap-3 items-start flex-col md:flex-row md:items-stretch">
                        <div className="flex-1 flex flex-col gap-2 w-full">
                          <Label>{t("fields.languageLabel")}</Label>
                          <Controller name={`languageEntries.${index}.language`} control={control} render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange}>
                              <SelectTrigger><SelectValue placeholder={t("fields.languagesPlaceholder")} /></SelectTrigger>
                              <SelectContent>
                        ...
                        <div className="flex-1 flex flex-col gap-2 w-full">
                          <Label>{t("fields.proficiencyLabel")}</Label>
                          <Controller name={`languageEntries.${index}.proficiency`} control={control} render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange}>
                              <SelectTrigger><SelectValue placeholder={t("fields.proficiencyPlaceholder")} /></SelectTrigger>
                              <SelectContent>
                        ...
                        {fields.length > 1 && (
                          <div className="flex flex-col gap-1 shrink-0 self-start md:self-stretch w-full md:w-auto">
                            <p className="text-sm font-medium text-muted-foreground opacity-0 pointer-events-none select-none hidden md:block">{t("fields.languageLabel")}</p>
                            <Button type="button" variant="outline" onClick={() => remove(index)} className="md:self-center">{t("removeLanguage")}</Button>
                          </div>
                        )}
                      </div>
                    ))}
              ...
              <div className="bg-card rounded-xl p-4 border shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="flex-shrink-0">
                    <ShieldCheck className="w-14 h-14 text-primary/20" />
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
                    <h3 className="font-semibold text-base">{t("privacyTitle")}</h3>
                    <p className="text-muted-foreground text-sm leading-relaxed">{t("privacyDescription")}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-t shadow-lg py-4 z-10">
          <div className="max-w-[960px] w-full mx-auto px-4 md:px-6">
            <div className="flex justify-end items-center">
              <Button type="submit" size="lg" onClick={handleSubmit(onSubmit, onValidationError)} className="gap-2">
                {t("continue")}
